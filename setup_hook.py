#!/usr/bin/env python3
"""
Setup hook to automatically manage zakaah in sites/apps.txt
- Adds zakaah to apps.txt during installation
- Removes zakaah from apps.txt during uninstallation
"""
...
def get_bench_root():
	...
	# Fallback: assume we're in apps/zakaah
	return Path(__file__).resolve().parent.parent


def ensure_app_in_apps_txt():
	"""Ensure zakaah is listed in sites/apps.txt - required for bench build and install."""
	try:
		...
		# Remove any existing zakaah entries (handle duplicates)
		apps = [app for app in apps if app != "zakaah"]
		
		# Add zakaah if not present
		if "zakaah" not in apps:
			apps.append("zakaah")
			with open(apps_txt_path, "w", encoding="utf-8") as f:
				f.write("\n".join(apps) + "\n")
			print("✅ Added zakaah to sites/apps.txt")
			return True
		else:
			return False
...
def remove_app_from_apps_txt():
	"""Remove zakaah from sites/apps.txt during uninstallation."""
	try:
		bench_root = get_bench_root()
		apps_txt_path = bench_root / "sites" / "apps.txt"
		...
		if len(apps) < original_count:
			with open(apps_txt_path, "w", encoding="utf-8") as f:
				f.write("\n".join(apps) + "\n")
			print("✅ Removed zakaah from sites/apps.txt")
			return True
